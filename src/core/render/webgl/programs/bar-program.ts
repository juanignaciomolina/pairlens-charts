/**
 * OHLC Bar Program — instanced WebGL2 rendering for Western-style OHLC bars.
 *
 * Each visible bar produces 3 instances:
 *   partType 0 = vertical range line (low -> high)
 *   partType 1 = left open tick (horizontal, left half)
 *   partType 2 = right close tick (horizontal, right half)
 *
...
export const BAR_FLOATS_PER_INSTANCE = 6
const BYTES_PER_FLOAT = 4
const INSTANCE_STRIDE = BAR_FLOATS_PER_INSTANCE * BYTES_PER_FLOAT
...
void main() {
  ...
  if (i_partType < 0.5) {
    // Vertical range line (low -> high)
    left = ndcX - wickHalfW;
    right = ndcX + wickHalfW;
    top = ndcHigh;
    bottom = ndcLow;
  } else if (i_partType < 1.5) {
    // Left open tick (horizontal line from left edge to center)
    left = ndcX - u_halfW;
    right = ndcX;
    top = ndcOpen + wickHalfW;
    bottom = ndcOpen - wickHalfW;
  } else {
    // Right close tick (horizontal line from center to right edge)
    left = ndcX;
    right = ndcX + u_halfW;
    top = ndcClose + wickHalfW;
    bottom = ndcClose - wickHalfW;
  }

  // Snap all edges to physical pixel centers
  left = snapX(left);
  right = snapX(right);
  top = snapY(top);
  bottom = snapY(bottom);
...
void main() {
  fragColor = v_isUp > 0.5 ? u_upColor : u_downColor;
}
`
...
export class BarProgram {
  ...
  private initializeGpuResources(): void {
    const gl = this.gl
    this.program = compileProgram(gl, BAR_VERT, BAR_FRAG)

    const vao = gl.createVertexArray()
    if (!vao) {
      throw new Error('Failed to create bar VAO')
    }
    this.vao = vao

    const quad = gl.createBuffer()
    if (!quad) {
      throw new Error('Failed to create bar quad buffer')
    }
    this.quadBuffer = quad

    const interleavedBuffer = gl.createBuffer()
    if (!interleavedBuffer) {
      throw new Error('Failed to create bar interleaved buffer')
    }
    ...
    if (!upColor || !downColor) {
      throw new Error('Missing bar uniform locations')
    }
  ...
  private initializeAttributes(): void {
    ...
    // All 6 instance attributes from one interleaved buffer with stride/offset
  ...
  updateInstances(firstInstance: number, instanceCount: number): void {
    ...
    gl.bufferSubData(
      ...
      firstInstance * BAR_FLOATS_PER_INSTANCE,
      instanceCount * BAR_FLOATS_PER_INSTANCE,
    )
  }
  ...
  drawInterleaved(
    ...
    const gl = this.gl
    const floatLength = instanceCount * BAR_FLOATS_PER_INSTANCE
